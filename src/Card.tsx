...
const WeatherCard = ({ day }: { day: DayForecast }): ReactElement => {
  const date = new Date(day.date);
  const dayName = date.toLocaleDateString('en-US', { weekday: 'long' });
  const monthName = date.toLocaleDateString('en-US', { month: 'long' });
  const dayNum = date.getDate();

  const iconImg = icons[day.icon] ?? cloudIcon;
...
const TodayData = ({ hour, isNow }: { hour: HourData; isNow: boolean }): ReactElement => {
  ...
  const iconImg = icons[hour.icon] ?? cloudIcon;
...
const HourlyForecast = ({ allHoursData }: { allHoursData: HourData[] }): ReactElement => {
  ...
  return (
    <div className="today-weather">
      {displayData.map((hour) => {
        const hourString = hour.time ?? '0:00';
        const [hourPart = '0'] = hourString.split(':');
        const hourValue = parseInt(hourPart, 10);

        return (
          <TodayData
            key={hour.id}
            hour={hour}
            isNow={hourValue === currentHour}
          />
        );
      })}
    </div>
